...
public class ExtentReporterNG extends TestBase implements IReporter {
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
		...
		extent.addSystemInfo("Host Name", "UI Testing").addSystemInfo("Environment", "Local Environment")
				.addSystemInfo("User Name", "Rajagopal Arumugam");
		...
		if (tests.size() > 0) {
			for (ITestResult result : tests.getAllResults()) {
				...
				if (result.getThrowable() != null) {
					test.log(status,
							test.addScreenCapture(BrowserInteractions.capture(result.getMethod().getMethodName()))
									+ result.getThrowable());
				} else {
